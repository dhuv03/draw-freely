...
export function CommandPalette({ onClose }: Props) {
  ...
  const commands = useMemo(() => [
    ...
    ...state.elements.filter((el) => !el.isDeleted).map((element, index) => ({
      label: `Find: ${element.name || element.text?.replace(/\s+/g, ' ').slice(0, 40) || `${element.type} ${index + 1}`}`,
      run: () => {
        dispatch({ type: 'SET_TOOL', tool: 'select' });
        dispatch({ type: 'SET_SELECTION', ids: element.groupId ? state.elements.filter((el) => el.groupId === element.groupId).map((el) => el.id) : [element.id] });
  ...
  ], [dispatch, state.elements, state.viewport.zoom]);
