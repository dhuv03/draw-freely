...
export function CanvasColorPicker({ value, onChange, swatches = CANVAS_SWATCHES, label = 'colour' }: { value: string; onChange: (color: string) => void; swatches?: string[]; label?: string }) {
  ...
  return <div className="canvas-color-picker" ref={wrapRef}>
    <div className="canvas-color-swatches">{swatches.map((color) => <button key={color} className={value.toLowerCase() === color.toLowerCase() ? 'active' : ''} style={{ background:color }} onClick={() => { setDraft(color.replace('#', '')); onChange(color); }} aria-label={`${label} ${color}`}>{value.toLowerCase() === color.toLowerCase() && <span>✓</span>}</button>)}<i/><button className="canvas-color-custom" style={{ background:value }} onClick={() => { setDraft(value.replace('#', '')); setOpen((shown) => !shown); }} aria-label={`Custom ${label}`} title={`Custom ${label} hex code`} /></div>
